 ...
 * @param {number} size
 */
let chunkArray = (arr, // input & processing Array
size = 1, // chunk size
//catch invalid sizes
 ...
 */
class ChesslyEvent extends Event {
...
let end_drag = event => {
    ...
    if (board && board.draggingFromsquare) { //dragging piece in same board
        ...
        if (piece.length > 1) {
            if (piece[0] === board.draggingPiece)
                piece[1].remove();
            else
                piece[0].remove();
        }
 ...
 */
let translateSquare = (sq, // square: 'D5'
 ...
 */
let attributes_to_parametersObject = (CE, // input: Custom Element
init = {}, // optional initial/default parameters  attr-key:value
attrs = CE.getAttributeNames() // attribute names to be processed
) => attrs.reduce((pars, attr) => ((pars[attr] = CE.getAttribute(attr) || undefined), // add parameter  attr-key:value
 ...
 * @param {*} {is:'white-king'}
 */
function SVG_chesspiece({ is, // white-pawn, black-rook etc
...
fillcolors = ["gold", "silver"] // fill queen crown-circles and other details
    ...
    let svg = `data:image/svg+xml` +
        ...
        `${`<defs><linearGradient id='a'><stop offset='50%' stop-color='#${stopcolors[0]}'/><stop offset='100%' stop-color='#${stopcolors[1]}'/></linearGradient></defs>` +
            ...
            {
                none: ``,
                [___PIECE_PAWN___]: `<path d='M397 467c2 170-86 221-128 250-34 24-25 83 20 86 28 1 277 2 306-1 51-6 53-67 20-88-53-32-136-69-139-247 0-33-79-30-79 0z'/>` +
                ...
                [___PIECE_ROOK___]: `<path d='M200 323s4-117 18-156c30-6 89-9 126-4 1 24-5 74-1 97 33-2 101 2 144-2 1-28-7-68 1-99 27-2 89-4 118 8-4 40 0 95 0 95l76 2-11 60c-95 36-412 30-471-1z'/>` + //rear buttress
                ...
                [___PIECE_BISHOP___]: `<path d='M436 217c11-18 39-51 68-67a511 511 0 0 1 134 339c1 63-32 126-53 149'/>` +
                ...
                [___PIECE_QUEEN___]: `<path d='M244 825l-50-166 433 1-49 170-334-5z'/>` +
...
[
    ...
    customElements.define(piece_is, class extends HTMLImageElement {
        ...
        connectedCallback() {
            ...
            if (this.board && this.board.is_interactive)
                this.make_piece_interactive();
        ...
        _set_IMG_src() {
            ...
            let parameters = attributes_to_parametersObject(this, {}, // optional default settings
        ...
        attack_from_square, layers = [this.board.layerDestinations] //for now second param is always set
        ) {
            this.calculate_piece_moves() // array [...,"B5","","A6X",...] with piece destinations and attack X info
                ...
                if (attack_from_square == this.at) {
                    ...
                    if (to_square[2] == ___SQUARE_ATTACK_MARKER___)
                        piece.attackFrom(this.at); // set image outline
        ...
        attributeChangedCallback(name, oldValue, newValue) {
            if (name == ___AT___)
                this._at_square = squarenameUpperCase(newValue);
            this._set_IMG_src();
        }
        get at() {
        ...
        calculate_piece_moves(square = this.at) {
            ...
            let recordDestination = (_sqData, _xz_char, // attack x/X or defend z/Z
            ...
            _checksquare, _noBlockingCheck) => {
                ...
                if (_checksquareData.piece) {
                    if ( /* owncolor */playdirection == _checksquareData.playdirection) {
                        _checksquare = recordDestination(_checksquareData, ___SQUARE_POTENTIAL_DEFEND_MARKER___, "set_defended_By");
                        this.defended_from.push(_checksquare);
                    }
                    else {
                        if (_attackMode) { //pawns forward moves can't attack
                            _checksquare = recordDestination(_checksquareData, ___SQUARE_POTENTIAL_ATTACK_MARKER___, "attacked_By");
                        }
                        this.attacked_from.push(_checksquare);
                    }
                    if (_noBlockingCheck)
                        _checksquare = _checksquare.toUpperCase();
                    //inbetweenPieces.push(checksquareData);
                    if (!_noBlockingCheck)
                        _isAfterBlockingPiece = _checksquareData;
                }
                else {
                    if (_isAfterBlockingPiece)
            ...
            _add_square = files[idxFile - 1] + ranksAscending[idxRank - 1], _noBlockingCheck = false) => {
                if (_add_square)
                    attack_or_defend(_add_square, _noBlockingCheck);
                //else this is at my own square
            };
            let straights = _ => {
                //idxFile and idxRank are locally scoped variables
                //so function can be used for any dimension board (ie. checkers is 10x10)
                while (idxRank++ < idxBorder - 1)
                    _add_destination_square(); //forward
                reset();
                while (idxFile++ < idxBorder - 1)
                    _add_destination_square(); //right
                reset();
                while (--idxRank)
                    _add_destination_square(); //back
                reset();
                while (--idxFile)
                    _add_destination_square(); //left
                reset();
                return _reachableSquares;
            };
            let diagonals = _ => {
                while (++idxFile < idxBorder && ++idxRank < idxBorder)
                    _add_destination_square(); //forward-right
                reset();
                while (++idxFile < idxBorder && --idxRank)
                    _add_destination_square(); //backward-right
                reset();
                while (--idxFile && --idxRank)
                    _add_destination_square(); //backward-left
                reset();
                while (--idxFile && ++idxRank < idxBorder)
                    _add_destination_square(); //forward-left
                reset();
                return _reachableSquares;
            };
            let _procesDestinations = (_piece_offset_destinations, // array [ [0,1] , [1,1] ]
            ...
            _piece_offset_destinations.map(loc => {
                let tocell = translateSquare(square, // current processing square
                playdirection * loc[0], // black or white piece offset
                playdirection * loc[1]);
                if (tocell) { // if offset location is within board
                    if (_add_tocell_as_destination)
                        _add_destination_square(tocell, true /* do not check for blocking pieces*/);
            ...
            this.destinations.push(...{
                [___PIECE_PAWN___]: _ => {
                    ...
                    let _pawnDiagonalMoves = _procesDestinations([[-1, 1], [1, 1]], // diagonal moves
                    ...
                    let _pawnAttacks = _pawnDiagonalMoves // diagonals moves only valid
                        ...
                        .map(squareData => attack_or_defend(squareData.square, // record destination square
                    ...
                    if (idxRank == 2 || idxRank == 7)
                        _pawnForwardMoves.push([0, 2]); // no need to check direction, illegal move is off the board anyway
                    return [
                        ..._pawnAttacks,
                        ..._procesDestinations(_pawnForwardMoves, false).filter(squareData => !(_isAfterBlockingPiece && squareData.piece)) // keep the destination pawn can goto
                ...
                .filter(square => square // delete all empty/false locations
    ...
    set_square_relations() {
        if (this.board) {
            ...
            let setElementRelations = (square_element, attrName) => {
                ...
                if (piece)
                    set_square_and_piece(piece); // square is
            };
...
let cssgrid = gap => `position:absolute;
  width:100%;
  height:100%;
...
customElements.define(ChesslyBoardLayer, class extends HTMLElement {
    ...
    clear_squares_with_from_attributes() {
        if (this.children.length) {
            this._squares_marked_from.forEach(square => {
                ...
                if (square_element)
                    square_element.removeAttribute(___ATTR_FROM___);
            });
        }
    ...
    filterFunc = (element, squareData) => {
        ...
        let squares = all_board_squares.map(square => {
            ...
            if (filterFunc(element, squareData))
                return element;
            else
                return false;
        });
...
customElements.define(ChesslyNameSpace + "-board", class extends HTMLElement {
    constructor() {
        super();
    ...
    connectedCallback() {
        ...
        if (boardwidth > 300)
            this.setProperty("squarefontsize", boardwidth / 45 + "px"); // size of A1..H8 square label
        //this.setProperty("border", "12px");
        if (this._initfen)
            this.setfen(this._initfen);
    }
    addlayer(id) {
    ...
    add_board_piece(piece_is, square, 
    //default layer:
    layer = this.layerPieces) {
        if (is_square(square)) {
            if (piece_is.length < 3)
                piece_is = FEN_translation_Map(piece_is);
    ...
    show_moves_piece_in_square(from_square) {
        ...
        if (this.is_interactive) {
            ...
            if (document.location.href.includes('fen'))
    ...
    setfen(fen) {
        ...
        fen.split("/").map((rank, idx) => {
            rank
                ...
                if (piece_is)
                    this.add_board_piece(piece_is, square);
            });
        });
        //once the pieces are on the board, calc underlying layers
    ...
    get fen() {
        ...
        [...this.layerSquares.children].map((sq, idx) => {
            if (idx && !(idx % ___SQUARECOUNT___))
                add("/"); // if new rank(row) add a /
            if (sq.piece) { // if piece in this square add fen character
                if (sq.piece.length && this.draggingPiece)
                    add(this.draggingPiece.fen); //   if dragging over another piece, use draggingPiece FEN
                else
                    add(sq.piece.fen); //   else piece FEN
            }
            else
                empty++; // else count empties
        });
    ...
    piece(fen_is = "K", //just any default
    selector = false // optional custom selector string
        ...
        if (is_square(fen_is)) { // selector is A1 to H8 square
            ...
            if (fen_is == '*')
                selector = '*';
            else if (fen_is.length < 2)
                fen_is = FEN_translation_Map.get(fen_is);
            if (!selector) {
                if (fen_is.includes("-"))
                    selector = `[is="${fen_is}"]`;
                else
    ...
    isCheck(check = char => this.piece(char).hasAttribute(___ATTACKED_BY___)) {
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        if (name == ___ATTR_FEN___) {
            if (this.board)
                this.setfen(newValue);
            else
                this._initfen = newValue;
        }
    ...
    dispatch(eventname, detail = {}) {
        ...
        this.dispatchEvent(new CustomEvent(ChesslyNameSpace + "-event", {
            bubbles: true,
            composed: true,
            detail
        }));
